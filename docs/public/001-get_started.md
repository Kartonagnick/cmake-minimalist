...
1. Для начала нужно клонировать репозиторий Minimalist в каталог проекта.  
   Для примера, пусть он будет установлен в каталог `C:\sample_project\deploy\cmake\minimalist`  

   ```
   git clone "https://github.com/Kartonagnick/cmake-minimalist.git" "C:\sample_project\deploy\cmake\minimalist"
   ```
